...
public class BinaryLogisticRegressionTest {
	...
	public void test() {
		...
		final List<Pair<Vector, Boolean>> dataset = new ArrayList<Pair<Vector, Boolean>>();
		for (int i = 0; i < 100; ++i)
			dataset.add(Pair.make((Vector) new DenseArrayVector(class1.sample(dim)), true));
		for (int i = 0; i < 100; ++i)
			dataset.add(Pair.make((Vector) new DenseArrayVector(class2.sample(dim)), false));
